...
func requestSSL(ctx context.Context, cfg Config) error {
	...
	if strings.EqualFold(os.Getenv("SKIP_SSL"), "true") || os.Getenv("SKIP_SSL") == "1" {
		fmt.Println("SKIP_SSL is set; skipping certbot request.")
		return nil
	}
	mail := fmt.Sprintf("admin@%s", cfg.Domain)
	if err := runCommand(ctx, "certbot", "certonly", "--standalone", "-d", cfg.Domain, "--non-interactive", "--agree-tos", "-m", mail); err == nil {
		return nil
	}

	fmt.Println("Standalone certbot failed; attempting webroot fallback.")
	if err := os.MkdirAll("/var/www/html", 0o755); err != nil {
		fmt.Printf("certbot fallback failed: %v (continuing without SSL)\n", err)
		return nil
	}
	if err := runCommand(ctx, "certbot", "certonly", "--webroot", "-w", "/var/www/html", "-d", cfg.Domain, "--non-interactive", "--agree-tos", "-m", mail); err != nil {
		fmt.Printf("certbot fallback failed: %v (continuing without SSL)\n", err)
	}
	return nil
}
