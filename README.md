...
## First-time VPS Bootstrap (Optional)

If your VPS is empty, run the bootstrap installer to install Go and build the app:
...
- Installs ZiVPN UDP module and configures default password list (`ZIVPN_PASSWORDS`, default `zi`)
...
- To skip SSL issuance (e.g., domain not pointed yet), set `SKIP_SSL=1` when running the installer.
- If standalone SSL fails, the installer will try a webroot fallback at `/var/www/html`.
